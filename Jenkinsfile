pipeline{
            ...
            stages{
                    stage('--Front End--'){
                            steps{
                                    sh '''
                                            image="35.178.235.187:5000/frontend:build-$BUILD_NUMBER"
                                            docker build -t $image /var/lib/jenkins/workspace/DnD_master/frontend
                                            docker push $image
                                            ssh 3.9.16.216 -oStrictHostKeyChecking=no  << EOF
                    ...
                    stage('--Service1--'){
                            steps{
                                    sh '''
                                            image="35.178.235.187:5000/rand1:build-$BUILD_NUMBER"
                    ...
                    stage('--Service2--'){
                            steps{
                                    sh '''
                                            image="35.178.235.187:5000/rand2:build-$BUILD_NUMBER"
                    ...
                    stage('--Back End--'){
                            steps{
                                    sh '''
                                            image="35.178.235.187:5000/backend:build-$BUILD_NUMBER"
